  ...
  return useQuery<FleetTenant[]>({
    ...
    queryFn: async () => {
      ...
      const [{ data: restaurants, error: rErr }, { data: orders, error: oErr }, { data: unreadMsgs }] = await Promise.all([
        supabase.from('restaurants').select('id, name, slug, tagline, currency, accepting_orders, suspended'),
        supabase
          .from('orders')
      ...
      type RestaurantRow = { id: string; name: string; slug: string; tagline: string | null; currency: string; accepting_orders: boolean; suspended: boolean }
