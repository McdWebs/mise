...
import {
  currencySymbol,
  formatPriceExact,
} from "@/features/guest/utils/formatPrice";
...
function PlanFormField({
  ...
  return (
    <div>
      <label className="block text-label text-ink-6 font-medium mb-1.5">
        {label}
      </label>
      {children}
    </div>
...
function PlanIncludesPicker({
  ...
  if (menuLoading && flat.length === 0) {
    return <p className="text-body-sm text-ink-6 py-2">Loading menu…</p>;
  }

  if (flat.length === 0) {
    return (
      <p className="text-body-sm text-ink-6 py-2">
        Add categories and items on the Menu page first, then you can attach
        them to a plan.
      </p>
    );
  }
            ...
            {unmatchedLabels.map((label, i) => (
              ...
              >
                <span
                  ...
                  title="Not linked to a current menu item"
                >
                  {label}
                  <span className="text-[11px] ml-1.5 text-ink-7">
                    (custom)
                  </span>
                </span>
                <button
                  ...
                  return (
                    ...
                    >
                      <button
                        type="button"
                        onClick={() =>
                          on ? removeItem(item.id) : addItem(item.id)
                        }
                      ...
                      >
                        ...
                        {!item.available && (
                          <span className="text-ink-7 font-normal ml-1.5">
                            · unavailable
                          </span>
                        )}
                      </button>
...
export function PlansPage({ restaurant }: PlansPageProps) {
  ...
  const [editing, setEditing] = useState<AdminPlan | null | undefined>(
    undefined,
  );
  ...
  const editingPlanId =
    typeof editing === "object" && editing ? editing.id : null;
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="bg-paper border border-paper-3 rounded-3 p-10 text-center">
          ...
          <p className="text-body-sm text-ink-7 mt-1">
            Create a plan to show guests a fixed-price deal at the top of your
            menu.
          </p>
        </div>
      ) : plans.length > 0 ? (
          ...
          {plans.map((plan) => (
            ...
            >
              <div className="min-w-0">
                ...
                {plan.description && (
                  <p className="text-body-sm text-ink-6 mt-0.5 truncate">
                    {plan.description}
                  </p>
                )}
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-7 py-5 space-y-4">
          ...
          {/* Price + Available hours on the same row */}
          <div className="flex flex-wrap items-end gap-x-5 gap-y-4">
            <PlanFormField label="Price per person">
              <div className="flex items-center gap-2 w-36 px-3 py-2.5 border-[1.5px] border-paper-4 rounded-2 focus-within:border-saffron transition-[border-color] duration-standard">
                <span className="text-ink-5 text-body font-mono">
          ...
          <PlanFormField label="What's included">
            <PlanIncludesPicker
              ...
              setIncludedItemIds={(up) =>
                setDraft((d) => ({
                  ...d,
                  includedItemIds:
                    typeof up === "function" ? up(d.includedItemIds) : up,
                }))
              }
              setUnmatchedLabels={(up) =>
                setDraft((d) => ({
                  ...d,
                  unmatchedLabels:
                    typeof up === "function" ? up(d.unmatchedLabels) : up,
                }))
              }
            />
          </PlanFormField>
          ...
          <div className="flex items-center justify-between p-4 bg-paper-2 rounded-3">
            ...
            >
              <span
                className={`absolute top-0.5 w-3.5 h-3.5 bg-paper rounded-full transition-[left] duration-hover ${draft.active ? "left-[14px]" : "left-0.5"}`}
              />
            </button>
          </div>
        </div>
